...
#import salty GPC data (Omnisec v5)
def gpc_data_import(filename):
    #specify column headers because Malvern are incompetent idiots
    col_headers=('Ret. Vol.', 'RI', 'UV', 'IV - DP', 'IV -  IP', 'Adjusted RI', 'Adjusted UV', 'Adjusted IV - DP', 'Adjusted IV -  IP', 'Molecular Weight', 'Cumulative Weight Fraction', 'Normalized Wt Fr', 'Normalized Mole Fraction', 'Conc.', 'Conc.2', 'WF/dLogM')
    #read data from their incredibly stupid output text file
    data = pd.read_csv(f'{filename}.txt', names=col_headers, skiprows=40, sep='\t')
    #save the data in a useful, useable manner
    data.to_csv(f'{filename}.csv', index=False, na_rep='')
    #get mw and WFdLogMW for processing
    mw = data['Molecular Weight']
    WFdLogMW = data['WF/dLogM']
    #remove empty rows
    mw = mw.dropna()
    WFdLogMW = WFdLogMW.dropna()
    #convert log10[mw] to mw
    mw = 10**mw 
    return(mw, WFdLogMW)
...
#make polymer labels more easily customizable (ask at beginning how many blocks in the polymer, what block 1 is, what DP is, then what block 2 is etc...)