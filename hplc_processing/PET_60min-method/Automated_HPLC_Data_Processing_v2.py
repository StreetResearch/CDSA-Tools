...
def hplc_py_processing(wavelength, filename):
    ...
    try:
        peaks = chrom.fit_peaks(buffer=1, prominence=0.15, param_bounds={'amplitude':[0.001, 100], 'scale':[0, np.inf]}, known_peaks={
                                                                        19.0 : {'width': 0.25}, 
                                                                        20.0 : {'width': 0.25}, 
                                                                        31.0 : {'width': 0.25}}) #add trimer and tetramer and TPA to this
    ...
    if wavelength == '284.75':
        ...
        HPLC_calibration = {'MHET': {'retention_time': 19.0, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}, #uses universal calibration from BHET and Dimer, R² = 0.998
                            'BHET': {'retention_time': 20.0, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}, #uses universal calibration from BHET and Dimer, R² = 0.998
                            'Dimer': {'retention_time': 31.0, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}} #uses universal calibration from BHET and Dimer, R² = 0.998
        try:
...
def process_HPLC_data_from_filelist(filelist):
    ...
    for x in filelist:
        ...
        #plot the graph
        plt.imshow(Loaded_HPLC_Data_T, cmap='viridis', interpolation='nearest', extent=[0,60,400,200], aspect=0.1)
        plt.xticks(xticks)
        plt.yticks(yticks)
        plt.xlabel('Retention Time (min)')
