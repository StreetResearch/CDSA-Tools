...
def hplc_py_processing(wavelength, filename):
    print(f'HPLC-py processing for {filename} in progress...')
...
def process_HPLC_data_from_filelist(filelist):
    ...
    for x in filelist:
        ...
        xticks = [0,5,10,15,20]
        #plot the graph
        plt.imshow(Loaded_HPLC_Data_T, cmap='viridis', interpolation='nearest', extent=[0,20,400,200], aspect=0.1)
        plt.xticks(xticks)
        plt.yticks(yticks)
        plt.xlabel('Retention Time (min)')
...
#fix 2D graph axis
#fix .csv filetypes on instrument
