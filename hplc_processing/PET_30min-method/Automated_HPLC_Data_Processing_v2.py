...
def load_HPLC_3D_Data (filename):
    ...
    #get sample name
    metadata_header = pd.read_csv(f'{filename}.csv', sep=',', header=0, index_col=0, on_bad_lines='skip')
    sample_name=list(metadata_header.loc['Sample Name'])
    sample_name=str(sample_name[0])
    print(f'The sample in {filename} is called {sample_name}')
    print(f'\033[0m Loaded PDA Data for {filename} successfully')
    return(data, data_T, sample_name)
...
def pick_chromatogram_peaks(HPLC_Trace, UV_wavelength, filename):
    ...
    print(list(map(float, peak_retention_time)))
    print('These peaks had the following maxima:')
    print(list(map(float, peak_maxima)))
    print('-------')
...
def hplc_py_processing(wavelength, filename):
    ...
    peaks = None
    try:
        peaks = chrom.fit_peaks(buffer=1, prominence=0.15, param_bounds={'amplitude':[0.001, 100], 'scale':[0, np.inf]}, known_peaks={
                                                                        15.0 : {'width': 0.25}, 
                                                                        15.3 : {'width': 0.25}, 
                                                                        20.4 : {'width': 0.25}}) #add trimer and tetramer and TPA to this
        #peaks = chrom.fit_peaks(buffer=50, prominence=0.02) 
    except ValueError as error:
        print(f'\033[1;33m {error}\n\033[0m')
        ...
        try:
            peaks = chrom.fit_peaks(buffer=1, prominence=0.02, param_bounds={'amplitude':[0.001, 100], 'scale':[0, np.inf]}, known_peaks={
                                                                        15.0 : {'width': 0.25}, 
                                                                        15.3 : {'width': 0.25}, 
                                                                        20.4 : {'width': 0.25}}) 
        except ValueError as error:
            print(f'\033[1;33m {error}\n\033[0m')
            print('nope, HPLC-py just doesn\'t like this trace. Giving up for now...')      
        #assess the fit of the peaks
        if peaks != None:
            try:
                chrom.assess_fit()
                print('-------')
    ...
    #Define the peaks of interest and their calibration curves for quantitation
    if wavelength == '284.75':
        #ADD TPA, Trimer, and Tetramer here!!
        HPLC_calibration = {'MHET': {'retention_time': 15.0, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}, #uses universal calibration from BHET and Dimer, R² = 0.998
                            'BHET': {'retention_time': 15.3, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}, #uses universal calibration from BHET and Dimer, R² = 0.998
                            'Dimer': {'retention_time': 20.4, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}} #uses universal calibration from BHET and Dimer, R² = 0.998
        try:
            quantitated_peaks = chrom.map_peaks(HPLC_calibration, loc_tolerance=0.2)
            print('According to the calibration, the following species were detected in the HPLC injection vial:')
            print(quantitated_peaks)
            print('-------')
        except AttributeError as error:
            print(f'\033[1;33m {error}\n\033[0m')
            print('Unfortunately the quantitation failed, trying with stricter bounds.')
            try:
                #ADD TPA, Trimer, and Tetramer here too!!
                HPLC_calibration = {'MHET': {'retention_time': 14.9, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}, #uses universal calibration from BHET and Dimer, R² = 0.998
                                    'BHET': {'retention_time': 15.3, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}, #uses universal calibration from BHET and Dimer, R² = 0.998
                                    'Dimer': {'retention_time': 20.4, 'slope': 3000000, 'intercept': -223727, 'unit': 'mM'}} #uses universal calibration from BHET and Dimer, R² = 0.998
                quantitated_peaks = chrom.map_peaks(HPLC_calibration, loc_tolerance=0.1)
                print('According to the calibration, the following species were detected in the HPLC injection vial:')
                print(quantitated_peaks)
                print('-------')
            except AttributeError as error:
                print(f'\033[1;33m {error}\n\033[0m')
                print('Sorry bud, the quantitation failed for this sample. But you can try quantitating manually from the peak integrations')
                pass          
        #save the quantitated peaks data
        try:
            quantitated_peaks.to_excel(f'{filename}/{filename}_HPLC-py_Fitted-Peaks_{wavelength}nm_Quantitated.xlsx')
            quantitated_peaks.to_csv(f'{filename}/{filename}_HPLC-py_Fitted-Peaks_{wavelength}nm_Quantitated.csv', sep=',', index=True)
...
def process_HPLC_data_from_filelist(filelist):
    ...
    for x in filelist:
        filename = x
        Loaded_HPLC_Data, Loaded_HPLC_Data_T, Loaded_HPLC_Data_SampleName = load_HPLC_3D_Data(filename)
        #create a subfolder to save data
        try:
            os.mkdir(f'{Loaded_HPLC_Data_SampleName}') #creating a subfolder
        except FileExistsError: #if subfolder already exists
            pass 
        #save 3D data as table
        Loaded_HPLC_Data_T.to_excel(f'{Loaded_HPLC_Data_SampleName}/{Loaded_HPLC_Data_SampleName}_PDA_Data.xlsx')
        Loaded_HPLC_Data_T.to_csv(f'{Loaded_HPLC_Data_SampleName}/{Loaded_HPLC_Data_SampleName}_PDA_Data.csv', sep=',', index=True)
        ...
        plt.savefig(f'{Loaded_HPLC_Data_SampleName}/{Loaded_HPLC_Data_SampleName}_PDA-Plot.png', dpi=600, transparent=True)
        plt.close()
        ...
        Trace_285nm = extract_HPLC_trace(Loaded_HPLC_Data, wavelength_to_plot_1, Loaded_HPLC_Data_SampleName)
        plot_HPLC_Chromatogram (Trace_285nm, wavelength_to_plot_1, Loaded_HPLC_Data_SampleName)
        wavelength_to_plot_2 = '259.63'
        Trace_260nm = extract_HPLC_trace(Loaded_HPLC_Data, wavelength_to_plot_2, Loaded_HPLC_Data_SampleName)
        plot_HPLC_Chromatogram (Trace_260nm, wavelength_to_plot_2, Loaded_HPLC_Data_SampleName)
        print('-------')
        #-----------------------

        #extract and plot UV spectrum from major peaks
        #find peaks from HPLC trace
        peak_integer_index, peak_retention_time, peak_maxima = pick_chromatogram_peaks(Trace_285nm, wavelength_to_plot_1, Loaded_HPLC_Data_SampleName)
        UV_spectra_table = extract_plot_UV_spectrum(Loaded_HPLC_Data, Loaded_HPLC_Data_T, peak_retention_time, Loaded_HPLC_Data_SampleName)
        print('-------')
        #-----------------------

        #process HPLC chromatogram at 285nm and 260nm using hplc-py
        try:
            HPLCpy_Peaks_1 = hplc_py_processing(wavelength_to_plot_1, Loaded_HPLC_Data_SampleName)                   
        except ValueError as error:
            print(f'\033[1;33m {error}\n\033[0m')
            print(f'Sorry pal, HPLC-py did not like {Loaded_HPLC_Data_SampleName} at {wavelength_to_plot_1}nm. The chromatogram trace shape is not compatible with HPLC-py.')
        try:
            HPLCpy_Peaks_2 = hplc_py_processing(wavelength_to_plot_2, Loaded_HPLC_Data_SampleName)        
        except ValueError as error:
            print(f'\033[1;33m {error}\n\033[0m')
            print(f'Sorry pal, HPLC-py did not like {Loaded_HPLC_Data_SampleName} at {wavelength_to_plot_2}nm. The chromatogram trace shape is not compatible with HPLC-py.')
        
        #sample complete
        print(f'Data processing complete for {Loaded_HPLC_Data_SampleName}. HOORAY!! =)')
        continue
...
#TO DO
#pull sample name from input data and rename the folder, possibly incorporate into results file
#make some variables seperately importable - like input retention time, uv wavelengths to study, etc to a text file and load from there
